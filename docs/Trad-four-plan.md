...
**Phase 1b — Harmonic annotator** (`python/leadsheet/annotator.py`)
...
**Phase 1b — Tonal area integration** (`python/leadsheet/tonal_areas.py`)
Maps roadmap `KeySpan` objects onto individual `ChordEvent` objects. Each chord gets `tonal_key` (pitch class 0-11), `tonal_mode` ('Major'/'Minor'/'Dominant'), and `tonal_scale` (frozenset of pitch classes). Convenience function `run_roadmap(ls, lib, parser)` runs the full pipeline: CYK parse → find_keys → assign_tonal_areas.

**Phase 1c — Note function classifier** (`python/leadsheet/annotator.py:note_function()`)
Classifies any MIDI pitch against the current chord as C (chord tone), L (color tone), S (scale tone), A (approach tone — chromatic half-step neighbor of any chord tone), X (outside), or NC (no chord).
